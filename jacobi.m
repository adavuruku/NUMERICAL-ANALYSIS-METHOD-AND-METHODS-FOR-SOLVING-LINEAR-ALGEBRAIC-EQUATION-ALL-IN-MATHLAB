function jacobi(matrix_in,right_v,sol_v,l_y,tol)
%**************************************************************************
%Title       -   Jacobi Method Of Solving Linear System Of Algebraic Eqtn. *
...
%**************************************************************************
    fprintf(' -----------------------------\n');
    fprintf(' Jacobi Iterative Method (SLE)\n');
    fprintf(' -----------------------------\n');
    t = cputime;
    ...
    for j = 1 : l_y
        if abs(grid_data(j,j)) < sum(abs(grid_data(j,[1:j-1,j+1:l_y])))
            matrix_eqtn = grid_data
            fprintf('Given Equation Cannot be solve Using Jacobi Method \nBecause is Not Diagonnally Dorminant.\n')
            return;
        end
    end
    %Display headings
    ...
    while err > tol
        for j = 1 : l_y
             %compute formular
             in_data(j) =   double((right_data(j) - grid_data(j,[1:j-1,j+1:l_y])*sol_data([1:j-1,j+1:l_y]))/grid_data(j,j));
        end
        %compute error
