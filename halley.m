function halley(in_l,conv, fu)
%****************************************************************
%Title  -   Halley's Method Of Estimating Roots Of Polynomials. *
...
err = 1;
fprintf(' ------------------------\n');
fprintf(' Halleys Iterative Method\n');
fprintf(' ------------------------\n');
t = cputime;
%differentiate
diff_f = diff(fu);
diff_s = diff(fu,2);
        ...
        while abs(err) > conv
           ...
           x_old = x_new;
           %implement formular
           x_new = double(in_l - ((2*func*func_a)/((2*func_a*func_a)- (func * func_b))));
           iter = iter + 1;
           in_l = x_new;
           if x_new ~= 0
