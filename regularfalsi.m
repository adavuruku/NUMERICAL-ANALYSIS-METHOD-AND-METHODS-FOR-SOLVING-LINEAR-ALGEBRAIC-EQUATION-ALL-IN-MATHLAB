function regularfalsi(in_l,in_u,conv, fu)
%********************************************************************
%Title  -   Regular Falsi Method Of Estimating Roots Of Polynomials.*
...
err = 1;
fprintf(' -------------------------------------\n');
fprintf(' Regular Falsi - False Position Method\n');
fprintf(' -------------------------------------\n');
t = cputime;
...
func_b = double(subs(fu,in_u));
    ...
    if (func_a*func_b) >= 0
        ...
        while abs(err) > conv
           ...
           x_old = x_new;
           %implement formular
           x_new = double(((in_l*func_b) - (in_u*func_a))/(func_b - func_a));
           iter = iter + 1;
           if x_new ~= 0
               err = double((x_new - x_old)/x_new)*100;
           end
           fun_xnew = double(subs(fu,x_new));
           test = fun_xnew * func_a;
           if test < 0
               in_u =  x_new;
           elseif test > 0
               in_l =  x_new;
