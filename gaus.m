function gaus(matrix_in,right_v,sol_v,l_y,tol)
%**************************************************************************
%Title       -   Gaus Siedel Method Of Solving Linear System Of Equation. *
...
%**************************************************************************
    fprintf(' ----------------------------------\n');
    fprintf(' Gaus Siedel Iterative Method (SLE)\n');
    fprintf(' ----------------------------------\n');
    t = cputime;
    ...
    while err > tol
        for j = 1 : l_y
             %compute formular
             in_data(j) =   double((right_data(j) - grid_data(j,[1:j-1,j+1:l_y])*in_data([1:j-1,j+1:l_y]))/grid_data(j,j));
        end
        %compute error
